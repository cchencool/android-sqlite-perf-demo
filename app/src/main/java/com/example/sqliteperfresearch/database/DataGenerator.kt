...
class DataGenerator {
    ...
    private fun makeRow(r: Random, now: Date, idx: Int): ContentValues {
        ...
        cv.put("date_birth", DATE_FMT.format(Date(now.time - (Math.abs(r.nextLong()) % 1_000_000_000_000L))))
        cv.put("date_event", DATE_FMT.format(Date(now.time + (Math.abs(r.nextLong()) % 9_460_800_000L))))
        cv.put("time_start", TIME_FMT.format(now))
        cv.put("time_end", TIME_FMT.format(Date(now.time + (Math.abs(r.nextLong()) % 7_200_000L))))
